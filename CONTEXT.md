...
- 阶段事件带**发出它的子图调用 id**：两个子图并行时，卡片按调用 id 归因，不按声明顺序。
...
- 监督器每次模型调用前对消息数组执行的**非 LLM** 规范化，固定 7 步顺序：配对修复（丢孤儿 tool_result / 补缺失 tool_result）→ microcompact（按工具名的结果摘要替换，保留最近 N 条）→ 单条结果 offload（超字节预算写盘、原位留引用）→ snip（按「输入预算」从最旧截断）→ 再跑一次配对修复。
...
- 单次模型调用允许消耗的**输入** token 上限 = 模型上下文窗口 − 输出预留（`maxCompletionTokens`）− 估算误差缓冲（`consolidationSafetyBuffer`）。
- 唯一事实源是模型上下文窗口（`provider.contextWindow`，未声明该窗口的模型保守回退 32k 并在 provider 构造时警告一次）；两个扣减项是固定值，不随窗口缩放。
- 消费者是「消息准备」的 snip 与「会话压缩」的容量判断（读取未归档消息、归档目标），两处共用同一条推导；**不**把 system prompt 与工具 schema 预留在内——它们的真实大小由压缩触发侧的测量负责。
- _Avoid_：叫它「上下文窗口」（那是模型能力，不是预算）；也不要与「压缩触发阈值」混用——后者是策略，可以低于预算。

### 压缩触发阈值（Consolidation Trigger）

- 会话规模达到多少 token 就触发「会话压缩」（滚动摘要；记忆提取同挂此事）的**策略值**。实际触发点 = `min(触发阈值, 输入预算)`。
- 与输入预算刻意分离：小窗口模型在自身容量处触发（先摘要，不溢出），大窗口模型保持固定节奏、不随窗口放大——否则 1M 窗口下摘要与记忆提取几乎永不发生。
- _Avoid_：当成模型事实（它不由窗口推导），或与「输入预算」互换使用。
...
- 模型需要导图结构（超出选中范围）时按需调用 `readMindmap`（原 `getMindmapContext`）读工具，主进程经反向 IPC 向渲染层实时拉取。
...
- 一次子图执行由谁发起：**AI 触发**（模型调用虚拟工具）与**手动触发**（用户手势）。
- 两者不是两条执行路径：执行都落在同一个子图节点上，共享主图的线程与流；差异只在发起者与本轮上下文来源。
- _Avoid_: 把手动触发描述成「另一套执行代码」。

### 子图运行（Subgraph Run）

- 主图一次运行内某个子图节点的执行。
- 两个子图以**编译后的图作为节点**挂进主图（as node），不是节点函数里嵌套 `invoke`：内部超步计入主图的共享递归预算，持久化由主图 checkpointer 承担，`getWriter()` 自动继承（无需透传）。
- 一轮里声明的每个调用各自成为一次分派：普通工具调用逐个去工具节点，每个子图调用去自己的子图节点；多个分派落在同一超步内并行（不排队）。
...
- 包含：`activeSessionId`、`chatMessages`、`sessions`、`busy`、`streamText`、`toolCards`、`stopRequested`、`lastUserMessageAt`。
- `stopRequested` 为流停止标记（`markStreamStopping` 置位，`end`/`error` 复位）；`lastUserMessageAt` 为最近一次用户输入时间，二者供胶囊投影覆盖流状态使用。
- 不携带进度文案或错误文案：AI 进行中的信息只在对话框（点点气泡 + 子图卡片的阶段轨迹），错误只进排障日志（见「桥」与「排障日志」）。
...
- 缺省值（`busy ?? false`、`streamText ?? ''` 等）收在 selector 内部，组件不感知。
...
- 节点的主内容，所有节点类型的通用属性，纯文本（无富文本；导图不使用 Lexical）。
- _Avoid_: label（内存 NodeData 的历史字段名）、text（旧协议草案的属性名）

### 折叠（collapsed）

- 节点的通用展示属性：折叠该节点后的整棵子树。
- 折叠只影响展示，子树在文件中完整保留；缺省即展开。
- _Avoid_: hidden、closed

### 图片资源（asset）

- 内嵌在 `.mindlane` 文件中的图片资源，节点按引用共享。
- 图片在插入/生成时即下载转 base64 内嵌；文件因此自包含，移动/复制文件不丢图。
- _Avoid_: imageUrl（历史形态：节点直接存 URL 字符串，已废弃）

### 纯树（Pure Tree）

- 导图结构是严格树：每个节点至多一个父节点，边由父子嵌套派生，禁止任意连线。
- _Avoid_: edge（显式边列表是历史 JSON 格式的产物，不再存在）

### 根节点（root）

- 每个导图恰好一个根节点，固定 id `root`。
- AI 定位回退链的锚点。
...
- 每个条目是一个 server 定义：`id`、显示名、icon、transport 类型、连接配置、认证方式（OAuth 工厂或自定义头注入）。
...
- OAuth token、DCR client 凭据、以及非 OAuth server 的连接凭据等敏感数据不属于用户态，经 `safeStorage` 加密后单独存放。

### MCP 连接凭据

- 用户为某个非 OAuth MCP server 手填的密钥集合，如飞书应用凭证与用户 token（UAT/TAT）、Obsidian 的 API key。
- 与 OAuth token 同等对待：不属于用户态，经 `safeStorage` 加密存于 userData 独立文件，不进 `settings.json`。
- 连接时由 server 定义通过 `createAuthHeaders` 注入为 HTTP 请求头（如 `Authorization`、`X-Lark-MCP-UAT`）。
- _Avoid_: MCP 密钥（与连接状态混为一谈，丢失"凭据 = 可注入的认证载荷"的含义）

### MCP 可选开关

- 用户启用/停用某个 MCP server 的唯一方式：设置面板中的"连接 / 断开"操作。
- "连接"触发该 server 的授权流程（OAuth server 为浏览器授权；非 OAuth server 为弹连接表单填凭据）；"断开"删除凭据并移除其工具。
...
- 附件即**建图意图**：轮次状态里的附件段落带「请根据此文档内容生成思维导图」的指令；附件内容对模型不可见，只有导图子图能解析它。
- 已知缺口：针对附件的问答（摘要、提问）未实现，不假装已支持。
...
- 按批次预算把 chunk 序列组合成 batch 序列的纯函数。
- 单个 chunk 超过预算时独占一个 batch（允许超限），不切割 chunk。

### 批次预算（Batch Budget）

- 文档导入的累加器（Batcher）中，单个 batch 允许容纳的最大文本量 = 当前模型 `contextWindow` × 40%，以 2 字符 ≈ 1 token 折算为字符数。
- `contextWindow` 来自 provider 目录中的模型条目（`ModelOption`），未填写时回退 32k。
- 40% 是刻意保留的裕量，同时覆盖 prompt 模板与模型输出开销，不做精确 token 计数；这是文档批处理的换算，与「输入预算」（消息侧、固定扣减项）是两套算法。
- _Avoid_：叫「上下文预算」——那个名字会与「输入预算」混为一谈；也不要把两侧的换算合并（一侧按比例留裕量，一侧按固定项扣减）。
