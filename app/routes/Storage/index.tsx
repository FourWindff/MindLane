...
import MapDisplayer, { MapDisplayerProps } from "@/features/map";
import React, {
  useCallback,
  useMemo,
  useRef,
...
export default function StorageRoute() {
  ...
  const { data, removeCard, addGroup, removeGroup, renameGroup, moveCard } =
    useStore();
                  ...
                  <View style={styles.content}>
                    ...
                    >
                      {folderData
                        ...
                        .reverse()
                        .map((card, index) => {
                          const cardData = loadJsonDataSync(
                            card.filepath,
                            newMap
                          );
                          return (
      ...
      >
        ...
        >
          {recordtypes
            .filter(
              (folder) =>
                folder !== DEFAULT_GROUP &&
                !data[folder]?.some(
                  (item) => item.filepath === selectedCard?.filepath
                )
            )
            .map((category, index) => (
              <Chip
