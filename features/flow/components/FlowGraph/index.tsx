...
export default function FlowGraph({ initalNodes, scale }: FlowGraphProps) {
  ...
  const graphMeta = useRef<{ width: number, height: number }>({ width: 0, height: 0 });
  ...
  // 计算连接线的函数
  const calculateConnections = useCallback((nodes: FlowNodeMeta[]): Connection[] => {
  ...
  const handleNodePositionChange = useCallback((nodeId: string, x: number, y: number) => {
    ...
    setConnections(prevConnections => {
      ...
      // 更新当前节点的父节点连接线
      const parentConnections = newConnections.filter(
        conn => conn.id.endsWith(`-${nodeId}`)
      );
