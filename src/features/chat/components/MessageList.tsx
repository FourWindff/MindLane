    ...
    <div className={cx('chat-float-messages', isEmpty && 'chat-float-messages--empty')}>
      ...
      {busy && (
        <div className="chat-float-bubble-row chat-float-bubble-row--ai">
          ...
          <div className={cx('chat-float-streaming-group', !streamingText && 'chat-float-streaming-group--thinking')}>
            {activeTools.length > 0 && (
              <div className="chat-float-tool-calls chat-float-tool-calls--active">
                {activeTools.map((name, i) => (
                  <div key={`${name}-${i}`} className="chat-float-tool-tag chat-float-tool-tag--active">
            ...
            <div className={cx('chat-float-bubble chat-float-bubble--ai chat-float-bubble--streaming', !streamingText && 'chat-float-bubble--thinking')}>
              {streamingText ? (
      ...
      {[...messages].reverse().map((msg, i) => (
        <div
          ...
          className={cx('chat-float-bubble-row', msg.role === 'user' ? 'chat-float-bubble-row--user' : 'chat-float-bubble-row--ai')}
        >
          {msg.role !== 'user' && (
            <div className="chat-float-avatar chat-float-avatar--ai">
              <Sparkles size={14} strokeWidth={1.8} />
