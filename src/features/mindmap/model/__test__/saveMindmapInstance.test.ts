...
describe('saveMindmapInstance', () => {
  beforeEach(() => {
    useWorkspaceStore.setState({
      syncAfterFileSaved: vi.fn().mockResolvedValue(undefined),
    })
  })

  it('persists a dirty instance and marks it clean', async () => {
    ...
    await expect(saveMindmapInstance(instance)).resolves.toBe(true)
    ...
    expect(useWorkspaceStore.getState().syncAfterFileSaved).toHaveBeenCalledWith('/b.mindlane')
  })
  ...
  it('keeps the instance dirty when it changes again during persistence', async () => {
    ...
    const saving = saveMindmapInstance(instance)
  ...
  it('is a no-op for a clean instance', async () => {
    ...
    expect(useWorkspaceStore.getState().syncAfterFileSaved).not.toHaveBeenCalled()
  })
  ...
  it('routes to onError and skips IPC when filePath is null', async () => {
    ...
    await expect(saveMindmapInstance(instance, { onError })).resolves.toBe(false)
