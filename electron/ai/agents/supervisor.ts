...
export function buildSystemPrompt(profileText: string, context?: MindmapContextData | null): string {
  const parts = [
...
export function createSupervisorNode(params: {
  model: BaseChatModel
  tools: StructuredToolInterface[]
  profileText: string
}) {
  const { model, tools, profileText } = params
  const modelWithTools = model.bindTools!(tools)

  return async (state: typeof AgentState.State): Promise<Partial<typeof AgentState.State>> => {
    const systemPrompt = buildSystemPrompt(profileText, state.context)
...
export function routeSupervisor(state: typeof AgentState.State): string {
